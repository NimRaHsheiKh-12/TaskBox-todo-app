...
from .hf_mcp_service import hf_mcp_service
...
class ChatService:
    ...
    async def _handle_general_request(self, message: str) -> str:
        ...
        # Check if Hugging Face MCP service is available
        if hf_mcp_service.is_available():
            # Use Hugging Face model to generate a response
            try:
                # Format the message for the AI model
                hf_response = await hf_mcp_service.generate_text(
                    prompt=message,
                    max_tokens=100,
                    temperature=0.7
                )

                # If the response seems relevant to task management, enhance it with Taskie's personality
                if any(keyword in message.lower() for keyword in ["task", "todo", "list", "organize", "schedule", "remind"]):
                    enhanced_response = f"{hf_response} By the way, I'm Taskie, your friendly task assistant! 😊 I can help you manage your tasks. Just let me know if you'd like to add, view, update, complete, or delete tasks!"
                    return enhanced_response
                else:
                    return hf_response
            except Exception as e:
                print(f"Error using Hugging Face service: {e}")
                # Fall back to default response if HF service fails
    ...
    async def _handle_fallback_response(self, user_id: str, message: str, current_tasks: List[Dict]) -> Dict[str, Any]:
        ...
        if hf_mcp_service.is_available():
            try:
                # Use Hugging Face model to generate a more contextual response
                hf_response = await hf_mcp_service.generate_text(
                    prompt=f"The user said: '{message}'. I'm not sure how to interpret this in the context of task management. Could you help clarify?",
                    max_tokens=100,
                    temperature=0.8
                )

                reply = f"{hf_response} By the way, I'm Taskie, your friendly task assistant! 😊 I can help you add, view, update, complete, or delete tasks. Just let me know what you'd like to do!"
                ...
                print(f"Error using Hugging Face service for fallback: {e}")
                # Fall back to default responses if HF service fails
