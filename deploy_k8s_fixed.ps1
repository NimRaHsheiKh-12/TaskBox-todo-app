...
if ($ClusterType -eq "docker-desktop") {
...
elseif ($ClusterType -eq "minikube") {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Host "Hyper-V failed, trying with VirtualBox driver..." -ForegroundColor Yellow
...
# Build Docker images (same for both cluster types)
...
if ($Environment -eq "dev") {
    ...
    if (!(Test-Path $valuesFile)) {
        ...
        # Create the content for the dev values file
...
try {
    ...
    Write-Host "Could not retrieve service information: $_" -ForegroundColor Yellow
}
...
if ($ClusterType -eq "minikube") {
    ...
    if ($frontendSvc -and $frontendSvc.spec.ports) {
        ...
        Write-Host "Frontend: Could not determine NodePort, check service configuration" -ForegroundColor Yellow
    }

    if ($backendSvc -and $backendSvc.spec.ports) {
        ...
        Write-Host "Backend: Could not determine NodePort, check service configuration" -ForegroundColor Yellow
    }
} else {
