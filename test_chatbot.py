    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(f"{base_url}/chat/process_public", json=read_payload) as response:
                ...
                if response.status == 200:
                    print("✓ Task reading test passed")
                else:
                    print("✗ Task reading test failed")
                    
        except Exception as e:
            print(f"✗ Error during task reading test: {e}")
        ...
        if 'result' in locals() and result.get("updated_tasks"):
            ...
            try:
                ...
                print(f"✗ Error during task completion test: {e}")
        else:
        ...
        if 'result' in locals() and result.get("updated_tasks"):
            ...
            try:
                ...
                print(f"✗ Error during task update test: {e}")
        else:
        ...
        if 'result' in locals() and result.get("updated_tasks"):
            ...
            try:
                ...
                print(f"✗ Error during task deletion test: {e}")
        else:
